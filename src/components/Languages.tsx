      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {languages.map((lang, index) => (
            <div
              key={lang.name}
              className="group flex flex-col sm:flex-row items-start sm:items-center gap-6 p-6 bg-background border border-border rounded-2xl hover:border-primary/50 transition-all duration-300"
            >
              <span className="text-4xl font-bold font-mono text-primary">{lang.abbr}</span>

              <div className="flex-1 w-full">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-xl font-semibold text-foreground">{lang.name}</h3>
                ...
                <p className="text-muted-foreground text-sm">{lang.detail}</p>
              </div>
            </div>
          ))}
        </div>
